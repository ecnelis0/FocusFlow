...
describe("ConceptTags", () => {
  ...
  it("says plainly when a question is filed under nothing", () => {
    ...
    expect(
      screen.getByText("Not filed under any concept yet."),
    ).toBeInTheDocument();
  });
  ...
  it("tags an existing question with a concept", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Tag with a concept" }),
    );
    await user.click(
      await screen.findByRole("button", { name: CONCEPTS[0].title }),
    );
  ...
  it("does not offer a concept the question already carries", async () => {
    ...
    renderWithQuery(
      <ConceptTags
        mistake={makeMistake({
          concepts: [{ id: "c1", title: CONCEPTS[0].title }],
        })}
      />,
    );
    await user.click(
  ...
  it("shows each tag as a link to the concept, and can remove it", async () => {
    ...
    expect(
      screen.getByRole("link", { name: CONCEPTS[0].title }),
    ).toHaveAttribute("href", "/concepts/c1");

    await user.click(
      screen.getByRole("button", { name: `Remove ${CONCEPTS[0].title}` }),
    );
  ...
  it("points at writing one when there are no concepts yet", async () => {
    ...
    expect(
      await screen.findByText(/No concepts written yet/),
    ).toBeInTheDocument();
